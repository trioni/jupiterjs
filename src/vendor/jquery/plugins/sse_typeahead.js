 ...
 *
 * FORKED: 2013-03-06, Oskar Nordgren (Creuna). Allowing precompiled templates
 */
                ...
                if (suggestions.length > 0) {
                    ...
                    utils.each(suggestions, function(i, suggestion) {
                        if( dataset.compiled ) {
                            ...
                            elBuilder.innerHTML = parentTemplate.replace('%body', dataset.template.render(suggestion));
                        } else {
    ...
    var TypeaheadView = function() {
        ...
        function TypeaheadView(o) {
            ...
            utils.each(this.datasets, function(key, dataset) {
                var parentTemplate = '<li class="tt-suggestion">%body</li>';
                if (dataset.template) {
                    dataset.compiled = typeof(dataset.template) === 'function';
                    if( dataset.compiled ) {
                        dataset.template = dataset.engine.compile(dataset.template);
                        return;
                    }
